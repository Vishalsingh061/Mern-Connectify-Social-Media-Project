...
const ProfileDetail = () => {
    ...
    useEffect(() => {
        const fetchProfile = async () => {
            try {
                const res = await fetch(`https://mern-social-media-vt8f.onrender.com/user/find/${id}`, {
                    headers: {
    ...
    useEffect(() => {
        const fetchProfilePosts = async () => {
            try {
                const res = await fetch(`https://mern-social-media-vt8f.onrender.com/post/find/userposts/${id}`)
    ...
    const handleFollowFunction = async () => {
        try {
            await fetch(`https://mern-social-media-vt8f.onrender.com/user/toggleFollow/${profile?._id}`, {
                headers: {
                    'Authorization': `Bearer ${token}`
                },
                method: "PUT"
            ...
            <div className={classes.wrapper}>
                <div className={classes.top}>
                    <div className={classes.topLeftSide}>
                        <img
                            src={profile?.profileImg ? `https://mern-social-media-vt8f.onrender.com/images/${profile?.profileImg}` : man}
