...
const SuggestedUsers = () => {
    ...
    useEffect(() => {
        const fetchSuggestedUsers = async () => {
            try {
                const res = await fetch(`https://mern-social-media-vt8f.onrender.com/user/find/suggestedUsers`, {
                    headers: {
    ...
    const toggleFollow = async (id) => {
        try {
            await fetch(`https://mern-social-media-vt8f.onrender.com/user/toggleFollow/${id}`, {
                headers: {
                    'Authorization': `Bearer ${token}`
                },
                method: "PUT"
